...
pub fn day11() {
  ...
  .map(|l| {
    l.chars().into_iter().map(|x| (x as i32 - '0' as i32)).collect::<Vec<i32>>()
  })
  .collect::<Vec<Vec<i32>>>();

  fn increase_energy(res: &mut Vec<Vec<i32>>, i: i32, j: i32) -> bool {// did flash
  ...
  fn flash(res: &mut Vec<Vec<i32>>, i: i32, j: i32) -> i32 {
    ...
    for (x, y) in [(i-1, j-1), (i-1, j), (i-1, j+1), (i, j-1), (i, j+1), (i+1, j+1), (i+1, j-1), (i+1, j)] {
