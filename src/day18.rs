    ...
    match number {
        Number::Literal(val) => {
            if *val >= 10 {
                ...
                std::mem::replace(
                    number,
                    Number::Nested(Box::new(InnerNumber {
...
fn reduce_number_with_explosion_inner(
    number_box: &mut Box<Number>,
    depth: i32,
) -> (bool, (i32, i32)) {
    let number = number_box.as_mut();
    match number {
        Number::Nested(inner) => {
            if depth == 4 {
                ...
                let (exploded, (l, r)) =
                    reduce_number_with_explosion_inner(&mut inner.left, depth + 1);
                if exploded {
                    return (true, (l, consume_value_right(&mut inner.right, r)));
                }

                let (exploded, (l, r)) =
                    reduce_number_with_explosion_inner(&mut inner.right, depth + 1);
                if exploded {
...
fn magnitude(number: &Box<Number>) -> i32 {
    match number.as_ref() {
        Number::Literal(v) => *v,
        Number::Nested(i) => (magnitude(&i.left) * 3) + (magnitude(&i.right) * 2),
    }
}
...
pub fn day18_1() {
    ...
    let numbers = input
        .into_iter()
        .map(|line| Box::new(parse_number(&line, 0).1))
        .collect_vec();
