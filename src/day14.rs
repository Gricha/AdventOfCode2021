...
pub fn day14_1() {
  ...
  input.into_iter().skip(2)
    .map(|x| {
      let (a, b) = x.split(" -> ").collect_tuple().unwrap();
      (a.to_string(), b.to_string())
    })
    .for_each(|(a, b)| { rules.insert(a, b);});
...
pub fn day14() {
  ...
  input.into_iter().skip(2)
    ...
    .for_each(|(a, b)| { 
      ...
      rules.insert(a, ([letter_1, middle].iter().collect(), [middle, letter_2].iter().collect()));
    });

  let mut current_mapping: HashMap<String, i64> = HashMap::new();
  for i in 1..template.len() {
    *current_mapping.entry((&template[i-1..=i]).to_string()).or_insert(0) += 1;
  }
