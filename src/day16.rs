...
impl Packet {
    ...
    fn evaluate(&self) -> i64 {
        match &self.packet_type {
            ...
            PacketInfo::Subpacket(header) => match header.type_id {
                0 => header.subpackets.iter().map(|x| x.evaluate()).sum(),
                1 => header.subpackets.iter().map(|x| x.evaluate()).product(),
                2 => header
                    .subpackets
                    .iter()
                    .map(|x| x.evaluate())
                ...
                3 => header
                    ...
                    .unwrap(),
                5 => {
                    if header.subpackets.get(0).unwrap().evaluate()
                        > header.subpackets.get(1).unwrap().evaluate()
                    {
                        1
                    } else {
                        0
