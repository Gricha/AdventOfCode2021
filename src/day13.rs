...
pub fn day13() {
  ...
  let dots = input.iter().filter(|x| !x.starts_with("fold"))
  .map(|l| {
    let (x_str, y_str): (&str, &str) = l.split(",").collect_tuple().unwrap();
    (x_str.parse::<i32>().unwrap(), y_str.parse::<i32>().unwrap())
  }).collect::<Vec<(i32, i32)>>();
  let instructions = input.iter().filter(|x| x.starts_with("fold"))
  .map(|l| {
    let trimmed = l.strip_prefix("fold along ").unwrap();
    let (direction_str, coordinate_str): (&str, &str) = trimmed.split("=").collect_tuple().unwrap();
