...
impl Signal {
  ...
  fn get_output(&self) -> i32 {
    ...
    let remainder: Vec<char> = "abcdefg".chars().collect::<HashSet<char>>().difference(&chars).cloned().collect();
    ...
    let middle_char_vec: Vec<char> = "abcdefg".chars().into_iter().collect::<HashSet<char>>().difference(&chars_0).cloned().collect();
    ...
    let top_left_char_vec: Vec<char> = "abcdefg".chars().into_iter().collect::<HashSet<char>>().difference(&all_found_set).cloned().collect();
    assert!(top_left_char_vec.len() == 1);
    let top_left = top_left_char_vec[0];
    let match_0 = assemble_sorted_word(vec![top, top_left, top_right, bottom_left, bottom_right, bottom]);
    ...
    let match_6 = assemble_sorted_word(vec![top, top_left, middle, bottom_left, bottom_right, bottom]);
    let match_7 = assemble_sorted_word(vec![top, top_right, bottom_right]);
    let match_8 = assemble_sorted_word(vec![top, top_left, top_right, middle, bottom_left, bottom_right, bottom]);
    let match_9 = assemble_sorted_word(vec![top, top_left, top_right, middle, bottom_right, bottom]);
    ...
    let values: Vec<i32> = self.values.clone().into_iter().map(|x| -> String {
...
pub fn day8() {
  ...
  for i in input {
    ...
    let patterns: Vec<String> = split_string(patterns_str).into_iter().map(|x| x.to_string()).collect();
    let values: Vec<String> = split_string(values_str).into_iter().map(|x| x.to_string()).collect();
