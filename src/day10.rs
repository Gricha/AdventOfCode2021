...
pub fn day10() {
    ...
    let mut res = input
        .into_iter()
        .filter_map(|l| {
            match l.chars().into_iter().fold(
                PatternMatch::Correct(Vec::new()),
                |pattern_stack, value| match pattern_stack {
                    PatternMatch::Incorrect(val) => PatternMatch::Incorrect(val),
                    PatternMatch::Correct(mut stack) => {
                        if openers.contains(&value) {
                            ...
                            match stack.last() {
                                ...
                                None => PatternMatch::Incorrect(value),
                            }
                        }
                    }
                },
