...
fn translate(
    p: &(i32, i32, i32),
    rotation: &(&(i32, i32, i32), &(i32, i32, i32)),
) -> (i32, i32, i32) {
    let pa = [p.0, p.1, p.2];
    (
        rotation.1 .0 * pa[rotation.0 .0 as usize],
        rotation.1 .1 * pa[rotation.0 .1 as usize],
        rotation.1 .2 * pa[rotation.0 .2 as usize],
    )
}
...
pub fn day19() {
    ...
    let rotations = REMAP
        .iter()
        .cartesian_product(TRANSLATE.iter())
        .collect_vec();
    ...
    while scanners_positioned.len() < len_scanners {
        for i in 0..len_scanners {
            for j in 0..len_scanners {
                ...
                let positioned = if scanners_positioned.contains(&i) {
                    i
                } else {
                    j
                };
                let unpositioned = if !scanners_positioned.contains(&i) {
                    i
                } else {
                    j
                };
                ...
                for rotation in rotations.iter() {
                    let unpositioned_beacons = unpositioned_beacons_raw
                        .iter()
                        .map(|x| translate(x, rotation))
                        .collect_vec();
                    for (x1, y1, z1) in positioned_beacons.iter() {
                        for (x2, y2, z2) in unpositioned_beacons.iter() {
                            ...
                            let potential_matches = unpositioned_beacons
                                .iter()
                                .collect::<HashSet<&(i32, i32, i32)>>();
                            let mut match_count = 0;
                            for (xx, yy, zz) in positioned_beacons.iter() {
                                if potential_matches.contains(&(
                                    xx + delta_vector.0,
                                    yy + delta_vector.1,
                                    zz + delta_vector.2,
                                )) {
                            ...
                            if match_count >= 12 {
                                ...
                                let absolute_vector = (
                                    delta_vector.0 + absolute_vectors[positioned].0,
                                    delta_vector.1 + absolute_vectors[positioned].1,
                                    delta_vector.2 + absolute_vectors[positioned].2,
                                );
                                absolute_vectors[unpositioned] = absolute_vector;
                                data_absolute[unpositioned] = unpositioned_beacons
                                    .iter()
                                    .map(|x| {
                                        (
                                            x.0 - absolute_vector.0,
                                            x.1 - absolute_vector.1,
                                            x.2 - absolute_vector.2,
                                        )
                                    })
                                    .collect_vec();
                            }
    ...
    let distinct = absolute_positions
        .iter()
        .collect::<HashSet<&(i32, i32, i32)>>();
    ...
    for i in 0..len_scanners {
        for j in 0..len_scanners {
            let manhattan = (absolute_vectors[i].0 - absolute_vectors[j].0).abs()
                + (absolute_vectors[i].1 - absolute_vectors[j].1).abs()
                + (absolute_vectors[i].2 - absolute_vectors[j].2).abs();
