...
pub fn day15_1() {
  ...
  let res = input.into_iter()
    .map(|l| l.chars().into_iter().map(|c| c as i32 - '0' as i32).collect_vec())
    .collect_vec();
  ...
  fn update_risk(path_cost: &mut Vec<Vec<i64>>, risk_map: &Vec<Vec<i64>>, i: usize, j: usize) {
    ...
    let smallest_cost = min(min(path_cost[i-1][j], path_cost[i+1][j]), min(path_cost[i][j-1], path_cost[i][j+1])) + risk_map[i][j];
